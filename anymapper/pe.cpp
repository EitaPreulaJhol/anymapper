#include "pe.hpp"

pe::pe::pe( void* image_buffer )
{
	image_base = image_buffer;

	pdos_header = ( PIMAGE_DOS_HEADER )image_buffer;

	if ( !is_dos_header_valid() )
		return;

	pnt_headers = ( PIMAGE_NT_HEADERS )
		( ( uint64_t )image_buffer + pdos_header->e_lfanew );

	if ( !is_nt_headers_valid() )
		return;

	image_size = pnt_headers->OptionalHeader.SizeOfImage;
}

pe::pe::~pe()
{
}

bool pe::pe::fix_sections( void* raw )
{
	const PIMAGE_SECTION_HEADER section = 
		IMAGE_FIRST_SECTION( this->pnt_headers );

	for ( 
		auto i = 0; 
		i < this->pnt_headers->FileHeader.NumberOfSections; 
		i++ )
	{
		// sections without raw data (e.g. .bss) are left zeroed, the
		// image buffer is zero-initialized by the caller
		if ( !section[ i ].SizeOfRawData )
			continue;

		memcpy(
			( void* )( ( uint64_t )image_base + section[ i ].VirtualAddress ),
			( void* )( ( uint64_t )raw + section[ i ].PointerToRawData ),
			section[ i ].SizeOfRawData );
	}

	return true;
}

bool pe::pe::relocate_image( uint64_t delta ) const noexcept
{
	if ( !pnt_headers )
		return false;

	const auto relocation_data =
		pnt_headers->OptionalHeader
		.DataDirectory[ IMAGE_DIRECTORY_ENTRY_BASERELOC ];

	// the image carries no relocation table, there is nothing to fix up.
	// this is valid for images linked with /FIXED (RELOCS_STRIPPED); such
	// an image can only run correctly at its preferred base address.
	if ( !relocation_data.Size || !relocation_data.VirtualAddress )
		return true;

	// already loaded at its preferred base address
	if ( !delta )
		return true;

	const auto image_base_addr = ( uint64_t )image_base;
	const auto relocation_start = 
		image_base_addr + relocation_data.VirtualAddress;
	const auto relocation_end = 
		relocation_start + relocation_data.Size;

	// the relocation directory must be fully contained in the image
	if ( relocation_end < relocation_start ||
		relocation_end > image_base_addr + image_size )
		return false;

	auto relocation_entry = 
		reinterpret_cast< PIMAGE_BASE_RELOCATION >( relocation_start );

	__try
	{
		while ( 
			( uint64_t )relocation_entry + sizeof( IMAGE_BASE_RELOCATION )
				<= relocation_end )
		{
			// malformed or terminating block
			if ( !relocation_entry->SizeOfBlock ||
				( uint64_t )relocation_entry + relocation_entry->SizeOfBlock
					> relocation_end )
				break;

			const auto ibr_size = sizeof( IMAGE_BASE_RELOCATION );

			const auto address = 
				image_base_addr + relocation_entry->VirtualAddress;
			const auto count = 
				( relocation_entry->SizeOfBlock - ibr_size ) / sizeof( uint16_t );
			const auto list = reinterpret_cast< uint16_t* >( 
				( uint64_t )relocation_entry + ibr_size );

			static_assert( sizeof( uint16_t ) == 2, 
				"must be 2, this is due to non-64bit" );

			for ( auto i = 0u; i < count; i++ )
			{
				const uint16_t type = list[ i ] >> 12;
				const uint16_t offset = list[ i ] & 0xFFF;

				if ( type == IMAGE_REL_BASED_DIR64 )
				{
					*( uint64_t* )( address + offset ) += delta;
				}
			}

			relocation_entry = 
				reinterpret_cast< PIMAGE_BASE_RELOCATION >( 
					( uint64_t )relocation_entry 
						+ relocation_entry->SizeOfBlock );
		}
	}
	__except ( EXCEPTION_EXECUTE_HANDLER )
	{
		return false;
	}

	return true;
}

bool pe::pe::resolve_imports( 
	pre_callback_t pre_callback,
	post_callback_t post_callback,
	bool ret_on_pre_fail,
	bool ret_on_post_fail ) const noexcept
{
	if ( !pnt_headers )
		return false;

	const auto import_data =
		pnt_headers->OptionalHeader
		.DataDirectory[ IMAGE_DIRECTORY_ENTRY_IMPORT ];

	// this image does not have imports
	if ( !import_data.Size )
		return true;

	if ( !import_data.VirtualAddress )
		return false;

	auto import_entry = 
		reinterpret_cast< PIMAGE_IMPORT_DESCRIPTOR >( 
			( uint64_t )pdos_header + import_data.VirtualAddress );

	while ( import_entry->FirstThunk )
	{
		const auto module_name =
			std::string(
				reinterpret_cast< char* >(
					( uint64_t )pdos_header + import_entry->Name ) );

		if ( pre_callback )
		{
			const bool result = pre_callback( module_name );

			if ( !result && ret_on_pre_fail )
				return result;
		}

		// FirstThunk is the IAT that must be filled with the resolved
		// addresses for the image to be able to call its imports
		auto first_thunk =
			reinterpret_cast< PIMAGE_THUNK_DATA64 >(
				( uint64_t )pdos_header + import_entry->FirstThunk );

		// OriginalFirstThunk (the lookup table) names the imports. Some
		// images do not provide it, in that case it aliases the IAT
		const auto lookup_rva = import_entry->OriginalFirstThunk ?
			import_entry->OriginalFirstThunk : import_entry->FirstThunk;

		auto lookup_thunk =
			reinterpret_cast< PIMAGE_THUNK_DATA64 >(
				( uint64_t )pdos_header + lookup_rva );

		while ( lookup_thunk->u1.AddressOfData )
		{
			// import by ordinal: there is no name to resolve
			if ( !( lookup_thunk->u1.Ordinal & IMAGE_ORDINAL_FLAG64 ) )
			{
				auto thunk_data =
					reinterpret_cast< PIMAGE_IMPORT_BY_NAME >(
						( uint64_t )pdos_header 
							+ lookup_thunk->u1.AddressOfData );

				if ( post_callback )
				{
					const bool result = post_callback(
						module_name,
						&first_thunk->u1.Function,
						thunk_data->Name );

					if ( !result && ret_on_post_fail )
						return result;
				}
			}

			first_thunk++;
			lookup_thunk++;
		}

		import_entry++;
	}

	return true;
}

bool pe::pe::valid() const noexcept
{
	return this->is_dos_header_valid() && this->is_nt_headers_valid();
}

bool pe::pe::is_dos_header_valid() const noexcept
{
	if ( !pdos_header )
		return false;

	return pdos_header->e_magic == IMAGE_DOS_SIGNATURE;
}

bool pe::pe::is_nt_headers_valid() const noexcept
{
	if ( !pnt_headers )
		return false;

	return pnt_headers->Signature == IMAGE_NT_SIGNATURE;
}

bool pe::pe::is_64bit_image() const noexcept
{
	return pnt_headers->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC;
}

bool pe::pe::valid_ptr( void* ptr ) const
{
	// TODO: implement
	return true;
}
